...
def check_python_version():
    section("Step 1/12: Python Version")
...
def setup_env_file(silent=False):
    ...
    if env_path.exists():
        ...
        if not silent:
            if not ask("Overwrite?", default=False):
                info("Keeping existing .env")
                return
    else:
        info(".env not found — creating template")
    env_path.write_text(ENV_TEMPLATE)
    ok(".env created — fill in your API keys before running")
...
def show_ai_recommendations(sys_profile):
    ...
    print()
    info("Download: python3 scripts/ai_modules.py --detect-hardware --select-model")

def print_summary(missing, outdated, sys_deps, install_ok, start_after=False):
    ...
    if optional_missing:
        info(f"{len(optional_missing)} optional packages not installed (run with --full)")
    print()
    if install_ok and not critical_missing:
        ...
        print(f"  {c('⚠ Installation incomplete — fix errors above', YELLOW)}")
...
def ensure_root(mode):
    if os.geteuid() == 0:
        return True
    warn("Root privileges required for system package installation")
    if mode == "silent":
        err("Silent mode requires root — run with sudo")
        return False
    if ask("Re-run with sudo?", default=True):
        script = Path(__file__).resolve()
        args = [f"SUDO_RELAUNCH=1", "sudo", sys.executable] + sys.argv
        os.execvp(args[1], args[1:])
    return False

# ─────────────────────────────────────────────
#  Kali system package install
# ─────────────────────────────────────────────
...
def install_apt_packages(missing_only=True):
    ...
    print()
    info(f"Installing {len(to_install)} system packages (this may take a while)...")
    try:
        ...
        if proc.returncode == 0:
            ...
            err(f"apt install failed (exit code {proc.returncode})")
            return False
    except Exception as e:
        err(f"apt install error: {e}")
        return False
...
def setup_postgresql():
    section("Step 5/12: PostgreSQL Setup")
    try:
...
def setup_venv():
    section("Step 7/12: Python Virtual Environment")
...
def setup_frontend():
    section("Step 8/12: Frontend Dependencies")
    if not shutil.which("npm"):
        warn("npm not found — skipping frontend setup")
        info("Install Node.js on Kali: sudo apt install nodejs npm")
        return
    try:
...
def build_installer(mode="interactive"):
    ...
    if mode != "silent" and not sys_profile.kali:
        err("Kali Linux not detected — this installer requires Kali Linux")
        err("See docs/kali_compatibility.md for requirements")
        sys.exit(1)

    if mode in ("interactive", "full"):
        if not ensure_root(mode):
            sys.exit(1)
    ...
    if mode == "silent":
        if not sys_profile.kali:
            err("Kali Linux required but not detected")
            sys.exit(1)
        # In silent mode, just run all steps automatically
        return _run_all_steps(sys_profile)

    if mode in ("update", "interactive"):
        do_update = (mode == "update")
        return _interactive_flow(sys_profile, do_update)

def _run_all_steps(sys_profile):
    info("Running in silent/automated mode")
    ...
    for name, cfg in SYSTEM_DEPS.items():
        try:
            subprocess.run(cfg["check"].split(), capture_output=True, text=True, timeout=5, check=True)
        except (FileNotFoundError, subprocess.TimeoutExpired):
            failed_sys.append(name)

    if failed_sys:
        section("Step 4/12: Installing Kali System Packages")
    ...
    section("Step 12/12: AI Model Info")
    show_ai_recommendations(sys_profile)

    print_summary(missing, outdated, {}, install_ok, start_after=False)
    _write_log(sys_profile, missing, outdated)
    return install_ok

def _interactive_flow(sys_profile, do_update=False):
    install_ok = True
    ...
    if do_update:
        pip_path = ".venv/bin/pip" if Path(".venv/bin/pip").exists() else sys_profile.pip_path
...
I need to finish handling the missing and outdated dependencies — installing critical ones, optionally installing optional packages, and upgrading outdated critical dependencies if confirmed. Then I'll set up the virtual environment and frontend dependencies, configure the .env file if needed, create the directory structure, run verification checks, display AI model recommendations, and print a summary of what was installed with options to start services.
<write_to_file>
<path>install.py</path>
...
def _run_all_steps(sys_profile, mode="silent"):
    silent = (mode in ("silent", "full", "update"))
