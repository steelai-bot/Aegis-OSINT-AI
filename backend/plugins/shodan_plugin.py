...
class ShodanPlugin(BasePlugin):
                ...
                for record in records:
                    if isinstance(record, dict):
                        ...
                        records_by_type[rtype].append(
                            {"value": record.get("value"), "last_seen": record.get("last_seen")}
                        )
    ...
    async def _organization_search(
        self, client, org_name: str, api_key: str
    ) -> list[PluginResponse]:
        """
    ...
    async def _vulnerability_search(self, client, query: str, api_key: str) -> list[PluginResponse]:
        ...
        try:
            ...
            url = f"{self._base_url}/shodan/host/search"
            params = {"key": api_key, "query": f'hostname:"{query}" vuln:*', "facets": "vuln:10"}
