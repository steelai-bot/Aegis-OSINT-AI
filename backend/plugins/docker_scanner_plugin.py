...
class DockerScannerPlugin(BasePlugin):
    ...
    def _check_docker_exposures(self, configs: list[dict]) -> dict:
        ...
        return {
            ...
            "credentials": "hardcoded_password" in vulnerabilities
            or "plain_text_passwords" in vulnerabilities,
    ...
    async def _check_ip_exposed_configs(self, ip: str) -> list[PluginResponse]:
        ...
        total_risk = (
            sum(1 for exp in ip_exposures if exp["risk"] == "HIGH")
            + (len(ip_exposures) - sum(1 for exp in ip_exposures if exp["risk"] == "HIGH")) * 0.5
        )
                ...
                evidence=[
                    {
                        ...
                        "critical_exposures": [
                            ...
                            if any(e["exposure"] == "database_credentials" for e in ip_exposures)
                            else None,
                            "Cache server credentials"
                            if any(e["exposure"] == "cache_access" for e in ip_exposures)
                            else None,
                        ],
                        "remediation": [
    ...
    async def _analyze_generic_docker_config(
        self, query: str, target_type: TargetType
    ) -> list[PluginResponse]:
